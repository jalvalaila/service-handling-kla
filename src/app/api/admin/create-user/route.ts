...
/**
 * Super-admin-only: create a teammate's login directly.
 * Requires SUPABASE_SERVICE_ROLE_KEY in Vercel — the anon key can't create users with a password.
 */
export async function POST(req: Request) {
  ...
  const { data: callerProfile } = await admin.from("profiles").select("role").eq("id", userRes.user.id).single();
  ...
  const branchScope = Array.isArray(body.branchScope)
    ? body.branchScope.filter((w: unknown) => typeof w === "string" && w.length > 0)
    : [];
  ...
  for (let i = 0; i < 5; i++) {
    const { error } = await admin
      .from("profiles")
      .update({ role, branch_scope: branchScope, is_active: true, full_name: fullName })
